#ifndef BALDZARIKA_UCV_SURF_HESSIAN_RESPONSE_H
#define BALDZARIKA_UCV_SURF_HESSIAN_RESPONSE_H

namespace baldzarika { namespace ucv { namespace surf {

	template < typename T >
	class hessian_response
	{
	...
	public:
		...
		hessian_response(math::size2ui const &sz, boost::uint32_t oc=4, boost::uint32_t ol=2, boost::uint32_t ss0=1)
		{
		...
		inline math::size2ui const& size() const
		{
			return m_size;
		}

		inline boost::uint32_t octaves() const
		{
			return m_octaves;
		}

		inline boost::uint32_t octave_layers() const
		{
			return m_octave_layers;
		}

		inline boost::uint32_t sample_step0() const
		{
			return m_sample_step0;
		}

		inline bool recreate(math::size2ui const &sz, boost::uint32_t oc=4, boost::uint32_t ol=2, boost::uint32_t ss0=1)
		{
			...
			boost::uint32_t middle_layers_size=m_octaves*m_octave_layers;
			
			m_layers.resize(layers_size);
			m_middle_layers.resize(middle_layers_size);
	...
	private:
		...
		std::vector<boost::uint32_t>	m_middle_layers;
	};
...
#endif //BALDZARIKA_UCV_SURF_HESSIAN_RESPONSE_H
#ifndef BALDZARIKA_UCV_JACOBI_H
#define BALDZARIKA_UCV_JACOBI_H

#include <baldzarika/ucv/matrix.h>
#include <baldzarika/ucv/vector.h>
#include <baldzarika/ucv/utils.h>
...
namespace baldzarika { namespace ucv {
	...
	bool jacobi(matrix<T, N, N> const &S0, vector<T, N> &e, matrix<T, N, N> &E, bool ce=true, T eps=std::numeric_limits<T>::epsilon())
	{
		...
		for(it=0;it<max_iters;++it)
		{
			...
			if(std::abs(p)<=eps)
				break;
			T y=(e(l)-e(k))*detail::constant::half<T>();
			T t=std::abs(y)+utils::hypot(p,y);
			T s=utils::hypot(p,t);
			T c=t/s;
			s=p/s; t=(p/t)*p;
			if(y<detail::constant::zero<T>())
				s=-s,t=-t;

			S(k,l)=detail::constant::zero<T>();
			e(k)-=t;
			e(l)+=t;
